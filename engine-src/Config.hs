...
read txt =
  let trimmedText :: Text
      trimmedText = T.strip txt
  in  case T.splitOn ";" <| trimmedText of
        ...
        _ -> Left <| UnexpectedConfigStructure trimmedText
...
throw error = case error of
  UnexpectedConfigStructure text ->
    ["The structure of the config was not what I expected -> ", text]
      |> T.concat

  BeatLengthIsntInt text ->
    ["This value isnt a valid beat length, its not an int ->", text] |> T.concat

  TimingVarianceIsntInt text ->
    ["This value isnt a valid timing variance, its not an int ->", text]
      |> T.concat

  ScaleError scaleError -> Scale.throw scaleError

  RoomError  roomError  -> Room.throw roomError
