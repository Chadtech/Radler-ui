...
instance Show Part where
  show part = case part of
    Sin        model -> "Sin " ++ show model

    Saw        model -> "Saw " ++ show model

    Harmonics  model -> "Harmonics " ++ show model

    DullSaw    model -> "Dull Saw " ++ show model

    Percussion model -> "Percussion " ++ show model

    Test       model -> "Test " ++ show model
...
diff (incomingPart, existingPart) = case (incomingPart, existingPart) of
  (Sin incomingSinModel, Sin existingSinModel) ->
    Osc.diff incomingSinModel existingSinModel |> Resolution.map Sin |> Right

  (Saw incomingSawModel, Saw existingSawModel) ->
    Osc.diff incomingSawModel existingSawModel |> Resolution.map Saw |> Right
  ...
  _ -> Right Resolution.Unresolvable


fromPieces :: Config -> (Text, List Text) -> Either Error Part
fromPieces config (partTxt, noteTxts) = case nameAndFields partTxt of
  Right ("sin", fields) ->
    noteTxts
      |> Osc.read config (Sin.makeFlags fields)
      |> Either.mapRight Sin
      |> Either.mapLeft SinError

  Right ("saw", fields) ->
    noteTxts
      |> Osc.read config (Saw.makeFlags fields)
      |> Either.mapRight Saw
      |> Either.mapLeft SawError

  Right ("harmonics", fields) -> case Harmonics.makeFlags fields of
    Right flags ->
      noteTxts
    ...
    Left error -> Left <| HarmonicsFlagsError error

  Right ("dullsaw", fields) ->
    noteTxts
      |> Osc.read config (DullSaw.makeFlags fields)
      |> Either.mapRight DullSaw
      |> Either.mapLeft DullSawError

  Right ("percussion", fields) ->
    noteTxts
      |> Percussion.read config (Percussion.makeFlags fields)
...
nameAndFields partText =
  case partText |> T.filter ((/=) ' ') |> T.splitOn "|" of
    ...
    _ -> Left VoiceInvalidFormat


readMany :: Config -> Text -> Text -> Either Error (List Part)
readMany config voiceNameTxts noteTxts =
  let voiceNames :: List Text
      voiceNames = voiceNameTxts |> T.strip |> T.splitOn ";"
      ...
      notesLength :: Int
      notesLength = List.length notes


      voicesLength :: Int
      voicesLength = List.length voiceNames
  in  if notesLength == voicesLength
        then
          notes
          |> List.zip voiceNames
          |> List.map (fromPieces config)
          |> CM.sequence
        else VoicesAndNotesNotOneToOne voicesLength notesLength |> Left


toDevAudio :: Part -> Audio
toDevAudio part = case part of
  Sin        model -> oscToDevAudio model

  Saw        model -> oscToDevAudio model

  Harmonics  model -> oscToDevAudio model

  DullSaw    model -> oscToDevAudio model

  Percussion model -> model |> Percussion.toMono False |> Audio.fromMono

  Test       model -> model |> Test.toMono |> Audio.fromMono


oscToDevAudio :: Osc.Model t -> Audio
oscToDevAudio = Osc.toMono .> Audio.fromMono


build :: Maybe Room -> Part -> Audio
build maybeRoom part = part |> buildUnclean maybeRoom |> Audio.trimEnd


buildUnclean :: Maybe Room -> Part -> Audio
buildUnclean maybeRoom part =
  let build_ :: Osc.Model t -> Audio
      build_ = Osc.build maybeRoom .> Audio.trimEnd
  in  case part of
        Sin       model -> build_ model

        Saw       model -> build_ model

        Harmonics model -> build_ model

        DullSaw   model -> build_ model

        Percussion model ->
          model |> Percussion.build maybeRoom |> Audio.trimEnd

        Test model -> model |> Test.build |> Audio.trimEnd
...
errorToText error = case error of
  UnrecognizedPartType txt -> T.append "unrecognized part type -> \n" txt
  ...
  SinError sinError ->
    sinError |> Osc.throw |> T.append "Error in Sin Voice -> \n"

  SawError sawError ->
    sawError |> Osc.throw |> T.append "Error in Saw Voice -> \n"

  HarmonicsError harmonicsError ->
    harmonicsError |> Osc.throw |> T.append "Error in Harmonics Voice -> \n"

  DullSawError dullSawError ->
    dullSawError |> Osc.throw |> T.append "Error in Dull Saw Voice -> \n"

  PercussionError percussionError ->
    percussionError |> Percussion.throw |> T.append
      "Error in Percussion Voice -> \n"

  TestError testError ->
    testError |> Test.throw |> T.append "Error in Test Voice -> \n"

  HarmonicsFlagsError harmonicsError ->
    harmonicsError |> Harmonics.throw |> T.append
      "Error making Harmonics Flags -> \n"

  DiffError -> "Diffing parts that arent the same type"
