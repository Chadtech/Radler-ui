...
instance Show Duration where
  show (Duration duration) =
    ["Duration ", T.pack <| show duration] |> T.concat |> T.unpack
...
read resolution txt = case TR.hexadecimal txt of
  Right (v, _) ->
    v
      *  resolution
...
fromFreqAndWaveCount freq count =
  (44100 / Freq.toFloat freq) |> (*) (toFloat count) |> floor |> Duration
...
throw error = case error of
  TextNotHexadecimal txt -> ["Duration is not hexadecimal : ", txt] |> T.concat
