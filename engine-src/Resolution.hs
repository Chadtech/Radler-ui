...
instance Show a => Show (Resolution a) where
  show resolution = case resolution of
    Identical    -> T.unpack "identical"

    Unresolvable -> T.unpack "unresolvable"

    Changes (remove, add) ->
      ["Changes", "Remove", T.pack <| show remove, "Add", T.pack <| show add]
        |> T.unlines
        |> T.unpack
...
map f resolution = case resolution of
  Identical     -> Identical

  Unresolvable  -> Unresolvable

  Changes tuple -> Tuple.both f tuple |> Changes
