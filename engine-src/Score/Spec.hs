...
makeNotes :: Text -> Text
makeNotes firstNote =
  ["0,1,", firstNote, ";1,2,30\n3,3,21;4,4,30\n"] |> T.concat


testText :: Text
testText = makeTestTextFromNotes <| makeNotes "20"


tests :: SpecWith ()
tests = case "20" |> makeNotes |> makeTestTextFromNotes |> Score.fromText of
  Right score -> Test.describe "score tests" <| do
    Test.specify "Score builds" <| do
      Score.toDevAudio score
        |> expect (Audio.fromList [10.0, 15.0, 0.0, 10.5, 15.0])

    Test.specify "Score diffed with itself resolves to identical" <| do
      Score.diff (Score.Incoming score) (Score.Existing score)
        |> expect (Right Resolution.Identical)

    Test.specify
        "Score diffed with slightly different version resolves to Changes"
      <| case "21" |> makeNotes |> makeTestTextFromNotes |> Score.fromText of
           Right score2 ->
             Score.diff (Score.Incoming score2) (Score.Existing score)
           ...
           Left error -> expect True False


  Left error -> Test.specify (show error) <| do
    expect True False
