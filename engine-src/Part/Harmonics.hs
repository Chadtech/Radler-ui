...
makeFlags fields = case Parse.get "harmonics" fields of
  Just harmonicsText ->
    harmonicsText
      |> T.splitOn ","
      |> List.map parseHarmonic
      |> CM.sequence
      |> Either.mapRight (Osc.Flags toMono fields <. Model)

  Nothing -> Left NoHarmonicsField
  ...
  case Parse.fromDelimitedText Parse.float harmonicText of
    Right harmonicsFields ->
      case
        ...
        of
          (Just vol, Just harmonic) -> Harmonic harmonic (Volume vol) |> Right


          (Nothing , _            ) -> Left NoVolume

          (_       , Nothing      ) -> Left NoHarmonic


    Left error -> Left <| HarmonicInvalid error


toMono :: Model -> Osc.Note -> Mono
toMono (Model harmonics) note =
  harmonics |> List.map (toHarmonicMono note) |> Mono.mixMany
...
toHarmonicMono note harmonic =
  Mono.sin 0
           ...
           (Osc.duration note)
    |> Mono.setVolume (Volume.multiply (volume harmonic) (Osc.volume note))
    |> Mono.declip
