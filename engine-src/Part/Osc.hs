...
instance Show (Model t) where
  show osc =
    [ osc |> notes |> Timeline.size |> show |> T.pack |> T.append
        "Number of Notes : "
...
read config flags noteTexts = case parseFreqError <| fields flags of
  Right maybeFreqError ->
    noteTexts |> (readManyNoteTexts config maybeFreqError) |> andThen
      (readModel flags)
...
parseFreqError fields = case Parse.get "freqerror" fields of
...
readManyNoteTexts
  :: Config -> Maybe Float -> List Text -> Either Error (List (Time, Note))
readManyNoteTexts config maybeFreqError noteTexts =
  readManyNoteTextsAccumulate config maybeFreqError noteTexts []
...
readManyNoteTextsAccumulate config maybeFreqError noteTexts notes =
  case noteTexts of
    first : rest -> case readNoteText config maybeFreqError first of
      Right (Just note) ->
        readManyNoteTextsAccumulate config maybeFreqError rest (note : notes)

      Right Nothing ->
        readManyNoteTextsAccumulate config maybeFreqError rest notes
...
readNoteText
  :: Config -> Maybe Float -> Text -> Either Error (Maybe (Time, Note))
readNoteText config maybeFreqError noteTxt = case Note.read config noteTxt of
  Right (time, seed, contentTxt) -> case contentTxt of
    "X" -> Right Nothing

    _   -> readNonEmptyNoteText config maybeFreqError (time, seed, contentTxt)
      |> Either.mapRight Just
...
readNonEmptyNoteText
  :: Config
  -> Maybe Float
  -> (Time, Random.StdGen, Text)
  -> Either Error (Time, Note)
readNonEmptyNoteText config maybeFreqError (time, seed, contentTxt) =
  let
      -- Something like "34"
    noteText :: Text
    noteText = slice 0 2 contentTxt

    -- 00 to FF
    volumeText :: Text
    volumeText = slice 4 6 contentTxt

    -- 00 to FF
    durationText :: Text
    durationText = slice 2 4 contentTxt

    -- Something like "n" "o" or "i"
    contourText :: Text
    contourText = slice 6 7 contentTxt

    freqResult :: Either Error Freq
    freqResult =
      case (Scale.toFreq (Config.scale config) noteText, maybeFreqError) of
        (Right freq, Just freqError) ->
          let (freqAdjustment, newSeed) =
                  (Random.randomR (-freqError, freqError) seed) :: ( Float
                    , Random.StdGen
                    )
          in  Freq.map ((*) (1 + freqAdjustment)) freq |> Right

        (Right freq , Nothing) -> Right <| freq

        (Left  error, _      ) -> Left <| ScaleError error
  in
    Note
    |> Right
    |> parse freqResult               id
    |> parse (Volume.read volumeText) VolumeError
    |> parse (Duration.read (Config.beatLength config) durationText)
             DurationError
...
toMono model =
  model
    |> notes
    |> Timeline.map (toMonoFromModel model <| subModel model)
    |> Timeline.toMono
...
build maybeRoom model =
  ...
  in  case (maybeRoom, position model) of
        (Just room, Just position) ->
          positionMono room position mono |> Audio.fromStereo
...
throw error = case error of
  ...
  ScaleError scaleError ->
    scaleError |> Scale.throw |> T.append "Scale Error ->\n"

  VolumeError    volumeError   -> Volume.throw volumeError

  DurationError  durationError -> Duration.throw durationError

  PositionError  positionError -> Position.throw positionError

  FieldsError    text          -> T.append "Fields Error -> " text

  FreqErrorError text          -> T.append "Freq Error parsing failed -> " text

  ContourError contourError ->
    contourError |> Contour.throw |> T.append "Contour Error -> "
