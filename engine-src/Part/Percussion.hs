...
instance Eq Model where
  (==) model0 model1 =
    (notes model0 == notes model1) && (position model0 == position model1)


instance Show Model where
  show osc =
    [ osc |> notes |> show |> T.pack
      , osc |> position |> show |> T.pack |> T.append "Position : "
      ]
      |> T.unlines
      |> T.unpack
...
instance Show Note where
  show note =
    [show <| volume note, " ", show <| seed note, " ", show <| sound note]
      |> List.map T.pack
      |> T.concat
      |> T.unpack
...
instance Show Sound where
  show sound = T.unpack <| soundToText sound
...
diff incomingModel existingModel =
  ( mapNotes (Timeline.filterKey (isntNoteOf incomingModel)) existingModel
...
read config flags noteTexts =
  noteTexts |> readManyNoteTexts config |> andThen (readModel flags)
...
parsePosition fields = case Parse.get "position" fields of
  Nothing           -> Right Nothing

  Just positionText -> case Parse.fromDelimitedText Parse.float positionText of
    Right fields ->
      fields
        |> Position.read
    ...
    Left error -> Left <| FieldsError error
...
readManyNoteTextsAccumulate config noteTexts notes = case noteTexts of
  first : rest -> case readNoteText config first of
    Right (Just note) -> readManyNoteTextsAccumulate config rest (note : notes)

    Right Nothing     -> readManyNoteTextsAccumulate config rest notes
...
readNoteText config noteTxt = case Note.read config noteTxt of
  Right (time, seed, contentText) -> case contentText of
    "X" -> Right Nothing

    "Q" -> Right <| Just (time, HardRest)

    _   -> readNonEmptyNoteText config (time, seed, contentText)
      |> Either.mapRight Just
...
readNonEmptyNoteText
  :: Config -> (Time, Random.StdGen, Text) -> Either Error (Time, Note)
readNonEmptyNoteText config (time, seed, contentText) =
  let
      -- Something like "pul"
      noteText :: Text
      noteText = slice 0 3 contentText

      -- 00 to FF
      volumeText :: Text
      volumeText = slice 3 5 contentText

      -- Extra info for specific notes
      extraText :: Text
      extraText = slice 5 (fromInt64 <| T.length contentText) contentText
  in  Note
        |> Right
...
readSoundText extraText soundText = case soundText of
  "pul" -> case Duration.read 10 extraText of
    Right duration -> Right <| Pulse duration

    Left  error    -> Left <| DurationError error

  "noi" -> case Duration.read 100 extraText of
    Right duration -> Right <| Noise duration

    Left  error    -> Left <| DurationError error

  "hat" -> case Duration.read 100 extraText of
    Right duration -> Right <| Hat duration
  ...
  _     -> Left <| UnrecognizedSoundText soundText


toMono :: Bool -> Model -> Mono
toMono building model =
  model |> notes |> Timeline.map (noteToMono building) |> Timeline.toMono


noteToMono :: Bool -> Note -> Mono
noteToMono building note = case note of
  HardRest               -> Mono.silence <| Duration (44100 * 30)

  Note volume seed sound -> case sound of
    Unit -> Mono.singleton <| Volume.toFloat volume

    Pulse duration ->
      volume |> Volume.toFloat |> Mono.fromSample duration |> Mono.declip

    Noise (Duration duration) ->
      let noise :: List Float
          noise = Random.randomRs (-1, 1) seed


          volumeBias :: Mono -> Mono
          volumeBias = if building then Mono.setVolume (Volume 2) else id
      in  noise
            |> List.take duration
            |> Mono.fromList
    ...
    Hat (Duration duration) ->
      let noise :: Mono
          noise =
              Random.randomRs (-1, 1) seed
                |> List.take duration
                |> Mono.fromList
                |> Mono.setVolume volume
                |> Mono.Fade.out Timing.Linear
      in  [ Mono.sin 0 (Freq 310) (Duration 1280)
    ...
    Clunk ->
      let
        pulseFadingIn :: Mono
        pulseFadingIn = Mono.fromSample (Duration 50) 0.2
          |> Mono.applyUntil 25 (Mono.Fade.in_ Timing.EaseInOut)
        ...
        (highFreq, seed1) =
          (Random.randomR (423, 479) seed) :: (Float, Random.StdGen)


        (lowFreq, seed2) =
          (Random.randomR (199, 221) seed1) :: (Float, Random.StdGen)
      in
        [ pulseFadingIn
    ...
    Kick ->
      let
        pulseFadingIn :: Mono
        pulseFadingIn = Mono.fromSample (Duration 10) 0.2
          |> Mono.applyUntil 25 (Mono.Fade.in_ Timing.EaseInOut)
        ...
        (highFreq, seed1) =
          (Random.randomR (115, 129) seed) :: (Float, Random.StdGen)


        (lowFreq, seed2) =
          (Random.randomR (36, 40) seed1) :: (Float, Random.StdGen)
      in
        [ pulseFadingIn
        , Mono.sinByWaveCount 0 (Freq highFreq) 3
        |> Mono.Fade.out Timing.Linear
...
build maybeRoom model =
  let mono :: Mono
      mono = toMono True model
  in  case (maybeRoom, position model) of
        (Just room, Just position) ->
          mono |> positionMono room position |> Audio.fromStereo

        _ -> Audio.fromMono mono
...
throw error = case error of
  ...
  VolumeError   volumeError   -> Volume.throw volumeError

  PositionError positionError -> Position.throw positionError
  ...
  FieldsError   text  -> T.append "Fields Error -> " text

  DurationError error -> Duration.throw error
