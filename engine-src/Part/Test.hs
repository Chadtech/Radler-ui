...
fromTexts config noteTexts =
    noteTexts
        |> fromTextsAccumulate config []
        |> Either.mapRight (Model <. Timeline.fromList)


fromTextsAccumulate :: Config -> List (Time, Note) -> List Text -> Either Error (List (Time, Note))
fromTextsAccumulate config notes noteTexts =
    case noteTexts of
        first : rest ->
            case fromNoteText config first of
                Right (Just note) ->
                    fromTextsAccumulate config (note : notes) rest

                Right Nothing ->
                    fromTextsAccumulate config notes rest
...
fromNoteText config noteText =
    case Note.read config noteText of
...
fromNonEmptyNoteText :: Time -> Random.StdGen -> Text -> Either Error (Time, Note)
