...
get routeTxt = Web.get (Web.capture (T.unpack routeTxt)) (withBody routeTxt)


post :: Text -> ScottyT Text Program ()
post routeTxt = Web.post (Web.capture (T.unpack routeTxt)) (withBody routeTxt)
...
withBody routeTxt = do
  ...
  let step acc = do
        ...
        if len > 0
          then chunk |> BS.unpack |> bytesToString |> (++) acc |> step
          else return acc
  bodyTxt <- liftIO <| step ""
...
respond route = Program.model |> andThen (update route)
