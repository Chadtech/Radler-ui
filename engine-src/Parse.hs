...
parse
  ...
  -> Either error (v -> w)
  -> Either error w
parse result errorCtor resultCtor = case resultCtor of
  Right ctor -> case result of
    Right v   -> Right <| ctor v

    Left  err -> Left <| errorCtor err

  Left err -> Left err


apply :: a -> Either e (a -> b) -> Either e b
apply a either = case either of
  Right f   -> Right <| f a
...
decode (Decoder parser) text =
  text |> TL.toStrict |> P.parseOnly parser |> Either.mapLeft TL.pack
...
listToFields = Fields <. Map.fromList <. List.map (Tuple.both TL.fromStrict)
...
get key (Fields map) = Map.lookup key map


int :: Decoder Int
int = Decoder <| P.signed P.decimal


float :: Decoder Float
float = Decoder <| realToFrac <$> P.signed P.double
