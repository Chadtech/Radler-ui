...
read config txt = case T.splitOn "," txt of
  timeTxt : randomSeedTxt : content : [] ->
    case (Parse.decodeInt timeTxt, Parse.decodeInt randomSeedTxt) of
      (Right time, Right randomSeed) ->
        ( time + (Config.timingVariance config) |> Time.fromInt
          , Random.mkStdGen randomSeed
          , content
          )
          |> Right

      (Right _, Left _ ) -> randomSeedTxt |> CouldntParseRandomSeed |> Left

      (Left  _, Right _) -> timeTxt |> CouldntParseTime |> Left

      (Left  _, Left _ ) -> CouldntParseAnything timeTxt randomSeedTxt |> Left

  _ -> txt |> NoteStringHadWrongStructure |> Left
...
throw error = case error of
  ...
  NoteStringHadWrongStructure txt -> T.append
