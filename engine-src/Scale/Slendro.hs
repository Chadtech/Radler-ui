...
toFreq txt = case Parse.decode Parse.int txt of
  Right int -> Either.mapRight
    (Freq.fromFloat <. ((*) (fundamentalsOctave int)))
    (fundamentalsTone int)

  Left _ -> Left <| NoteIsntInt txt



fundamentalsOctave :: Int -> Float
fundamentalsOctave int = (2 ^ quot int 10) * Constants.fundamental0 |> toFloat
...
fundamentalsTone int =
  ...
  in  case toneInt of
        ...
        _ -> Left <| ToneIsntInScale toneInt
...
throw error = case error of
  ...
  ToneIsntInScale int ->
    ["The Slendro scale only goes from 0 to 4, but I got ", T.pack (show int)]
      |> T.concat
