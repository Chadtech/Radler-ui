...
require ma fm = andThen (\f -> fmap f ma) fm
...
slice a b = T.take (fromIntegral (b - a)) <. T.drop (fromIntegral a)


replaceChar :: Char -> Char -> Text -> Text
replaceChar target replacement =
  T.pack <. map (replaceIfTarget target replacement) <. T.unpack
...
trace msg x = mapTrace msg id x


mapTrace :: Show b => String -> (a -> b) -> a -> a
mapTrace msg f x = Debug.trace (msg ++ " : " ++ (show <| f x)) x
