...
positionMono room soundPosition mono_ =
  let !mono_ = mono_
  in
    ( positionRelativeToEar (Room.mapListenerPosition leftEarPosition room)
                            soundPosition
                            mono_
      , positionRelativeToEar (Room.mapListenerPosition rightEarPosition room)
                              soundPosition
                              mono_
      )
      |> Stereo.fromMonos
...
leftEarPosition = Position.addToX (-0.5)


rightEarPosition :: Position -> Position
rightEarPosition = Position.addToX 0.5


positionRelativeToEar :: Room -> Position -> Mono -> Mono
positionRelativeToEar room soundPosition mono =
  let roomSize :: Size
      roomSize = Room.size room

      earPosition :: Position
      earPosition = Room.listenerPosition room

      !mono_      = mono
  in  [ directDelay earPosition soundPosition mono_
      , delayFromFarWallReflection roomSize earPosition soundPosition mono_
      , delayFromRightWallReflection roomSize earPosition soundPosition mono_
      , delayFromCeilingReflection roomSize earPosition soundPosition mono_
      , delayFromBackWallReflection earPosition soundPosition mono_
      , delayFromLeftWallReflection earPosition soundPosition mono_
      , delayFromFloorReflection earPosition soundPosition mono_
      ]
        |> Mono.mixMany
...
delayFromBackWallReflection earPosition soundPosition =
  let positionOnBackWall :: Position
      positionOnBackWall = reflectionPointOnBackWall earPosition soundPosition
  in  delayAndDecay
...
reflectionPointOnBackWall earPosition soundPosition =
  let relativeYDistanceFromBackWall :: Float
      relativeYDistanceFromBackWall =
          Position.x earPosition
            / (Position.x earPosition + Position.x soundPosition)
  in  ( Position.x earPosition
      + relativeYDistanceFromBackWall
...
delayFromFloorReflection earPosition soundPosition =
  let positionOnFloor :: Position
      positionOnFloor = reflectionPointOnFloor earPosition soundPosition
  in  delayAndDecay
...
reflectionPointOnFloor earPosition soundPosition =
  let relativeZDistanceFromFloor :: Float
...
delayFromLeftWallReflection earPosition soundPosition =
  let positionOnLeftWall :: Position
      positionOnLeftWall = reflectionPointOnLeftWall earPosition soundPosition
  in  delayAndDecay
...
reflectionPointOnLeftWall earPosition soundPosition =
  let relativeXDistanceFromLeftWall :: Float
...
delayFromCeilingReflection roomSize earPosition soundPosition =
  let positionOnCeiling :: Position
      positionOnCeiling =
...
reflectionPointOnCeiling roomSize earPosition soundPosition =
  let earZPositionFromCeiling :: Float
      earZPositionFromCeiling = Size.height roomSize - Position.z earPosition

      relativeZDistanceFromCeiling :: Float
      relativeZDistanceFromCeiling =
          earZPositionFromCeiling
            / ( earZPositionFromCeiling
              + (Size.height roomSize - Position.z soundPosition)
              )
  in  ( Position.x earPosition
      + relativeZDistanceFromCeiling
      * (Position.x soundPosition - Position.x earPosition)
...
delayFromRightWallReflection roomSize earPosition soundPosition =
  let positionOnRightWall :: Position
      positionOnRightWall =
...
reflectionPointOnRightWall roomSize earPosition soundPosition =
  let earXPositionFromRightWall :: Float
      earXPositionFromRightWall = Size.width roomSize - Position.x earPosition

      relativeXDistanceFromRightWall :: Float
      relativeXDistanceFromRightWall =
          earXPositionFromRightWall
            / ( earXPositionFromRightWall
              + (Size.length roomSize - Position.x soundPosition)
              )
  in  ( Size.width roomSize
      , Position.y earPosition
      + relativeXDistanceFromRightWall
      * (Position.y soundPosition - Position.y earPosition)
      , Position.z earPosition
...
delayFromFarWallReflection roomSize earPosition soundPosition =
  let positionOnFarWall :: Position
      positionOnFarWall =
...
reflectionPointOnFarWall roomSize earPosition soundPosition =
  let earYPositionFromFarWall :: Float
      earYPositionFromFarWall = Size.length roomSize - Position.y earPosition

      relativeYDistanceFromFarWall :: Float
      relativeYDistanceFromFarWall =
          earYPositionFromFarWall
            / ( earYPositionFromFarWall
              + (Size.length roomSize - Position.y soundPosition)
              )
  in  ( Position.x earPosition
      + relativeYDistanceFromFarWall
      * (Position.x soundPosition - Position.x earPosition)
...
delayAndDecay distance = Mono.delay (calcDelay distance)
  <. Mono.setVolume (Volume (1 / (distance ^ 2)))


calcDelay :: Float -> Duration
calcDelay distance = (round (distance / Constants.speedOfSound)) |> Duration
