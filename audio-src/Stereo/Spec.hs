...
one :: Stereo
one =
    Stereo.singleton (1,1)


zero :: Stereo
zero =
    Stereo.singleton (0,0)
...
tests :: SpecWith ()
tests = 
    Test.describe "Stereo" <| do
        Test.it "apends" <| do
            Stereo.append one zero
                |> Stereo.toList
                |> expect [ (1,1), (0,0) ]
        ...
        Test.describe "TrimEnd" <| do
            ...
            Test.it "wont trim anything off the end of a stereo that doesnt end in zero" <| do
                Stereo.append
                    onezerozero
                    one
                    |> Stereo.trimEnd
        ...
        Test.it "mixes" <| do
            Stereo.mix
                (Stereo.fromList [(0,0), (1,0), (-0.5, 0.1)])
